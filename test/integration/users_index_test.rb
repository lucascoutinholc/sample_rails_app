...
class UsersIndexAdminTest < UsersIndexAdmin
  ...
  test "should have delete links" do
    User.where(activated: true).paginate(page: 1, per_page: 25).each do |user|
      ...
      unless user == @admin
        assert_select 'a[href=?]', user_path(user), text: 'Excluir usuário'
      end
    end
  end
  ...
  test "should display only activated users" do
    User.paginate(page: 1, per_page: 25).first.toggle!(:activated)
...
class UsersNonAdminIndexTest < UsersIndex
  test "should not have delete links as non-admin" do
    log_in_as(@non_admin)
    get users_path
    assert_select "a", text: "Excluir usuário", count: 0
  end
end
